...
### Conversione RGB $\to$ YUV
La relazione tra RGB e YUV è definita da una trasformazione lineare (moltiplicazione matriciale).

1. **Luminanza (Y):** È una somma pesata dei canali RGB. Il peso maggiore è assegnato al **Verde** (circa il 59%) perché l'occhio umano è massimamente sensibile a quella frequenza per percepire la luminosità.
2. **Crominanza (U, V):** Sono definite come segnali di **differenza colore**:
   * **U** è proporzionale a $(B - Y)$ (Blu meno Luminanza).
   * **V** è proporzionale a $(R - Y)$ (Rosso meno Luminanza).
   * *(Il verde è ricavato per differenza dagli altri due).*

Matematicamente (riferimento standard BT.601):
$$
